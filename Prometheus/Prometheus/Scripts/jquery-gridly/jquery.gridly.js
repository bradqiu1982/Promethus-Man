...
(function() {
  ...
  Gridly = (function() {
    ...
    Gridly.prototype.size = function ($element) {
      var w = $element.data('width') || $element.width();
      return ((w <= 95 ? 95 : w) + this.settings.gutter) / (this.settings.base + this.settings.gutter);
    };
    ...
    Gridly.prototype.layout = function() {
      ...
      var max_y = 0;
      for (var m = 0; m < structure.positions.length; m++) {
        if (structure.positions[m].y != 0 && max_y < structure.positions[m].y) {
            max_y = structure.positions[m].y;
        }
      }
      for (index = _i = 0, _ref1 = $elements.length; 0 <= _ref1 ? _i < _ref1 : _i > _ref1; index = 0 <= _ref1 ? ++_i : --_i) {
        ...
        $element.css({
          ...
          left: position.x,
          //top: position.y,
          top: Math.round(position.y / max_y) * max_y,
        });
      }
      return this.$el.css({
