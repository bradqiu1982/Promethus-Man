    ...
    public class UserBlogVM
    {
        ...
        public static UserBlogVM RetrieveBlogDoc(string dockey)
        {
            ...
            foreach (var line in dbret)
            {
                ...
                {
                    var tempstrs = ret.Title.Split(new string[] { "-" }, StringSplitOptions.RemoveEmptyEntries);
                    var datestr = tempstrs[tempstrs.Length - 1].Substring(0, 8);
                    ret.DocURL = "/userfiles/docs/" + datestr + "/" + ret.Title;
                }
            }
        ...
        public static void StoreUserTag(string username, string tag)
        {
            ...
            {
                if (!existtag.Contains(tg.Trim().ToUpper()))
                {
